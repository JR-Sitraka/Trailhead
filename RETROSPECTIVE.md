...
| 4 | Framework misdetection fix | **Re-scoped mid-phase (ADR-010)** from "fix" to "verify + regression-guard" — the original defect (`got` misreported as Express) did not reproduce against current code | The retrospective finding that motivated this item does not reproduce against current code. Its historical accuracy and cause cannot be established from current evidence — only present non-reproduction is proven. Still a real caution about time-sensitive findings in retrospectives/ADRs. |
...
| 6 | Screen-reader accessibility | Shipped, merged — a live, first-time-user NVDA audit produced 7 findings: 6 confirmed defects, all fixed and independently live-reconfirmed, plus 1 reported Overview-heading defect that did not reproduce on investigation | None from scope, but the audit found real defects (Chat's total response silence, both Dashboard modals leaking Tab focus into browser chrome) that no automated tooling in the project had ever caught. |
| 7 | Testing closeout | Shipped — verified IMPORT-04 and dispositioned it as deliberately deferred; closed PREPROC-03 and the remaining planning-era Dashboard/Explorer rows | **Expanded significantly during execution.** Found and fixed three real production bugs that predated this entire phase and had nothing to do with the original closeout list: `boxen`'s NUL-byte import poisoning (silent false-success), `got`'s orphaned analyzing-state (unreconcilable, out-of-band data loss), and a real bypass of the 500MB import safety budget. One scoped, deliberate deferral: IMPORT-04 (branch selection) was found to be a real, fully unimplemented acceptance criterion — documented honestly as deferred, not marked passed. |
...
observation):**
- Item 6's screen-reader audit — all 7 findings, and the fixes
  for the 6 confirmed defects, both via the person's own first-time
  NVDA sessions
